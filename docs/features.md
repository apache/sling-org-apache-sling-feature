...
* A clash of artifacts (such as bundles) between the prototype and the feature is not resolved and both versions end up in the featuree. Artifact clashes are detected based on Maven Coordinates, not on the content of the artifact. So if a prototype defines a bundle with artifact ID `org.sling:somebundle:1.2.0` and the feature itself declares `org.sling:somebundle:1.0.1` in its `bundles` section, version `1.0.1` and version `1.2.0` are used. The removals section can be used to remove the prototype bundle. See [feature aggregation](aggregation.md) for the override rules that resolve such clashes during aggregation, and the optional `Bundle-SymbolicName` collision detection that catches same-BSN/different-Maven-coordinates pairs the GAV-based check cannot see.
